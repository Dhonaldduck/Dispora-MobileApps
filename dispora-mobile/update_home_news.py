...
# 1. Add shimmer and news_provider imports
if "import 'package:shimmer/shimmer.dart';" not in content:
    content = content.replace("import 'news/news_detail_screen.dart';", "import 'news/news_detail_screen.dart';\nimport 'package:shimmer/shimmer.dart';\nimport '../../features/news/providers/news_provider.dart';\nimport 'package:intl/intl.dart';")
...
# wait, I need to match the exact end of SafeArea to add closing for RefreshIndicator
pattern_safearea_close = r"          \),\n        \),\n      \),\n    \);\n  \}\n\}"
new_safearea_close = "          ),\n        ),\n      ),\n      ),\n    );\n  }\n}"
content = re.sub(pattern_safearea_close, new_safearea_close, content)

# 3. Replace _NewsSection
new_news_section = """class _NewsSection extends ConsumerWidget {
  const _NewsSection();
...
pattern = re.compile(r'class _NewsSection extends StatelessWidget \{.*?\n\}\n(?=class _ReportBanner)', re.DOTALL)
content = pattern.sub(new_news_section + '\n\n', content)
